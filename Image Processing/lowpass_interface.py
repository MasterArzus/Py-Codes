...
import cv2
import numpy as np
from views.Ui_Low_Filter_Interface import Ui_Low_Filter_Interface

class Low_Filter_Interface(Ui_Low_Filter_Interface, QWidget):
    ...
    def Gaussian_low_pass_filter_help(original_image, sigma=1):
    ...
    def Gaussian_low_pass_filter(self):
        ...
        transformed_img = self.Gaussian_low_pass_filter_help(img, alpha)

        self.show_transformed_image(transformed_img)

    def Butterworth_low_pass_filter(self, original_image, cutoff_frequency=10, order=2):
