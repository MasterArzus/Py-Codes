...
import cv2
import numpy as np
# from views.Ui_GammaInterface import Ui_Gamma_Interface


class Gamma_Interface(Ui_Gamma_Interface, QWidget):
    ...
    def slider_change(self):
        ...
        self.value_label.setText(str(self.gamma))
        if self.imgpath == '':
            return
        else:
            self.gamma_correct()
    ...
    def gamma_correct_help(self, image, gamma):
        # 归一化像素值到 [0, 1] 范围
    ...
    def gamma_correct(self):
        ...
        transformed_img = self.gamma_correct_help(img, alpha)
